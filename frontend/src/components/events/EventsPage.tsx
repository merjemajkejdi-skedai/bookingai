...
function EventFormModal({
  event, specialists, isArtClass, prefillDate, onClose, onSaved,
}: {
  event?: ArtEvent | null;
  specialists: Specialist[];
  isArtClass: boolean;
    ...
    <Modal title={event ? 'Edit Event' : 'New Event'} onClose={onClose}>
      <div className="flex flex-col gap-4">
        <Input
          ...
          placeholder="e.g. Watercolour Workshop"
          autoFocus
        />
        ...
        {isArtClass && (
          <div className="grid grid-cols-2 gap-3">
            <Input label="Age min" type="number" min={0} max={99} value={form.ageMin} onChange={e => set('ageMin', e.target.value)} placeholder="e.g. 3" />
            <Input label="Age max" type="number" min={0} max={99} value={form.ageMax} onChange={e => set('ageMax', e.target.value)} placeholder="e.g. 6" />
          </div>
        )}
...
function EventDetailPanel({
  ...
  const loadRegs = useCallback(async () => {
    setLoadingRegs(true);
    try {
      setRegs(await api.getRegistrations(event.id));
    } catch { /* panel still useful without regs */ }
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-start justify-between gap-2 pb-4 border-b border-slate-100">
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          <button onClick={onEdit}  className="p-1.5 rounded-lg text-slate-400 hover:bg-slate-100 hover:text-brand-500 transition-colors" title="Edit"><Pencil size={15} /></button>
          <button onClick={onDelete} className="p-1.5 rounded-lg text-slate-400 hover:bg-red-50 hover:text-red-500 transition-colors" title="Delete"><Trash2 size={15} /></button>
          <button onClick={onClose} className="p-1.5 rounded-lg text-slate-400 hover:bg-slate-100 transition-colors" title="Close"><X size={15} /></button>
        </div>
      </div>
...
export function EventsPage({ specialists, isArtClass }: EventsPageProps) {
  ...
  // Returns the freshly loaded events so callers can use the data synchronously
    ...
    <div className="flex flex-col h-full bg-white rounded-2xl border border-slate-200 overflow-hidden">
      ...
      {specialists.length > 1 && (
        <div className="md:hidden flex gap-2 overflow-x-auto px-3 py-2 border-b border-slate-100 flex-shrink-0 scrollbar-none">
          <button onClick={() => setFilterTeacherId(null)}
            className={clsx(
              'px-3 py-1 rounded-full text-xs font-medium border transition-colors whitespace-nowrap flex-shrink-0',
          ...
          {specialists.map(sp => (
            <button key={sp.id}
              onClick={() => setFilterTeacherId(sp.id === filterTeacherId ? null : sp.id)}
              className={clsx(
                'px-3 py-1 rounded-full text-xs font-medium border transition-colors whitespace-nowrap flex-shrink-0 flex items-center gap-1.5',
      ...
      <div className="flex flex-1 min-h-0">
        ...
        {selectedEvent && (
          <div className={clsx(
            'border-l border-slate-100 overflow-y-auto p-4 flex-shrink-0',
      ...
      {editingEvent && (
        <EventFormModal
          ...
          onSaved={async () => {
            ...
            // Update selectedEvent with the fresh data from server
