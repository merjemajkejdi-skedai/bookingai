...
// Art Events page — isArtClass is always false for this module
const IS_ART_CLASS = false;
...
function EventFormModal({
  event, prefillDate, onClose, onSaved,
}: {
  event?: ArtEvent | null;
  prefillDate?: string;
  onClose: () => void;
...
function AddRegistrationModal({ eventId, onClose, onSaved }: {
  ...
  const [form, setForm] = useState({ participantName: '', parentPhone: '', notes: '' });
  const [saving, setSaving] = useState(false);
  const [error, setError]   = useState('');
  ...
  return (
    <Modal title="Add Registration" onClose={onClose}>
      <div className="flex flex-col gap-4">
        <Input label="Name *" value={form.participantName} onChange={e => set('participantName', e.target.value)} autoFocus />
        <Input label="Phone" type="tel" value={form.parentPhone} onChange={e => set('parentPhone', e.target.value)} placeholder="+355691234567" />
        <label className="flex flex-col gap-1 text-sm">
...
// ── EventDetailPanel ──────────────────────────────────────────────────────────
function EventDetailPanel({ event, onClose, onEdit, onDelete, onRegistrationChange }: {
  event: ArtEvent; onClose: () => void; onEdit: () => void;
...
export function EventsPage({ specialists }: EventsPageProps) {
  ...
  const loadEvents = useCallback(async (): Promise<ArtEvent[]> => {
    ...
    try {
      ...
      setLoadError(e.message || 'Failed to load events'); return [];
