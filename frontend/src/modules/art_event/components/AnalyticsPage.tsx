...
export function AnalyticsPage() {
  ...
  function exportCSV() {
    if (!data) return;
    const rows: (string | number)[][] = [['Category', 'Name', 'Events', 'Participants', 'Revenue (ALL)']];
    data.byTemplate?.forEach((r: any) => rows.push(['Event Type', r.title, r.events, r.participants, r.revenue / 100]));
    data.byTeacher?.forEach((r: any)  => rows.push(['Teacher',    r.name,  '-',     r.participants, r.revenue / 100]));
    data.byEvent?.forEach((r: any)    => rows.push(['Event',      `${r.title} ${r.date}`, '1', r.participants, r.revenue / 100]));
          ...
          <>
            <div className="grid grid-cols-2 gap-3">
              ...
              <StatCard icon={<Users size={16} className="text-violet-500" />}
                label="Total participants" value={String(data?.overview?.totalParticipants ?? 0)} />
            </div>

            {data?.byTemplate?.length > 0 && (
              <Section title="By Event Type" icon={<BookOpen size={14} />}>
                <div>
                  ...
                  {data.byTemplate.map((r: any, i: number) => (
                    <div key={r.title ?? i}
                      ...
                      <div className="col-span-2 min-w-0">
                        ...
                        <p className="text-xs text-slate-400">{r.events} event{r.events !== 1 ? 's' : ''}</p>
                      </div>
            ...
            {data?.byEvent?.length > 0 && (
              <Section title="Events" icon={<CalendarDays size={14} />}>
                <div>
                  <div className="grid grid-cols-4 px-4 py-2 text-xs font-medium text-slate-400 uppercase tracking-wide border-b border-slate-100">
