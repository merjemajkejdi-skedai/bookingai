...
import { Clock, User, Scissors, FileText, AlertCircle, CheckCircle, XCircle } from 'lucide-react';
    ...
    try {
      if (isNew || mode === 'create') {
        await api.createBooking({ specialistId, serviceId, customerName, customerPhone, notes,
          startsAt: startsAt.slice(0, 16) + ':00', recurrence });
      } else {
          ...
          <>
            ...
            {booking.status === 'confirmed' && (
              <div className="flex gap-2 pt-2 border-t flex-wrap">
                ...
                <Button variant="outline" size="sm" onClick={() => handleStatusChange('no_show')}>No-show</Button>
                {!showCancelConfirm ? (
                  ...
                  <div className="flex flex-col gap-2 w-full mt-2">
                    ...
                    <input type="text" placeholder="Reason (optional)"
                      value={cancelReason} onChange={(e: any) => setCancelReason(e.target.value)}
                    ...
                    <div className="flex gap-2 flex-wrap">
                      ...
                      <Button variant="ghost" size="sm" onClick={() => setShowCancelConfirm(false)}>Keep booking</Button>
                    </div>
                    {booking?.recurrenceGroupId && (
          ...
          <>
            ...
            {(isNew || mode === 'create') && (
              <div className="flex flex-col gap-1.5">
                <label className="font-medium text-slate-700 text-sm">Repeat</label>
            ...
            {suggestions.length > 0 && (
              <div className="space-y-1.5">
                ...
                {suggestions.map(s => (
                  <button key={s.startsAt} onClick={() => { setStartsAt(s.startsAt.slice(0, 16)); setSuggestions([]); setError(''); }}
            ...
            <div className="flex gap-2 pt-1 border-t">
              {mode === 'edit' && <Button variant="ghost" size="sm" onClick={() => setMode('view')}>Back</Button>}
