...
export function AnalyticsPage({ isSalon }: Props) {
  ...
  function exportCSV() {
    if (!data) return;
    const rows: (string | number)[][] = [['Category', 'Name', 'Bookings', 'Revenue (ALL)']];
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <StatCard icon={<Users size={16} className="text-violet-500" />}
                label="Total bookings" value={String(data?.overview?.totalBookings ?? 0)} />
            </div>

            {/* By specialist */}
            {data?.bySpecialist?.length > 0 && (
              <Section title="By Specialist" icon={<Users size={14} />}>
                <Table rows={data.bySpecialist} colorDot />
              </Section>
            )}

            {/* By service */}
            {data?.byService?.length > 0 && (
              <Section title="By Service" icon={<Scissors size={14} />}>
                <Table rows={data.byService} colorDot />
              </Section>
            )}

            {/* By service group (saloon only) */}
            {isSalon && data?.byServiceGroup?.length > 0 && (
              <Section title="By Service Group" icon={<FolderOpen size={14} />}>
                <Table rows={data.byServiceGroup} />
              </Section>
            )}
            ...
            {!data?.bySpecialist?.length && !data?.byService?.length && (
              <div className="text-center py-16 text-slate-400">
                ...
                <p className="text-sm">No confirmed bookings in this period.</p>
              </div>
            )}
          </>
        )}
...
function Table({ rows, colorDot }: { rows: any[]; colorDot?: boolean }) {
  return (
    <div>
      ...
      {rows.map((r: any, i: number) => (
        <div key={r.id ?? r.name ?? i}
          ...
          <div className="flex items-center gap-2 min-w-0">
            {colorDot && <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: r.color ?? '#94a3b8' }} />}
            <span className="truncate text-slate-700">{r.name}</span>
          </div>
          <span className="text-center text-slate-600 font-medium">{r.bookings}</span>
