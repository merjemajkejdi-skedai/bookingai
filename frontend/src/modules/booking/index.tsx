      ...
      <aside className="hidden md:flex w-56 flex-shrink-0 flex-col bg-white border-r border-slate-200">
        ...
        {page === 'calendar' && (
          <div className="px-3 pb-4 border-t border-slate-100 pt-3">
            <p className="text-xs font-medium text-slate-400 mb-2 px-1">Filter by {specialistLabel.toLowerCase().slice(0, -1)}</p>
            <button
              ...
              All {specialistLabel.toLowerCase()}
            </button>
            {specialists.map(sp => (
              <button key={sp.id}
      ...
      <main className="flex-1 min-w-0 overflow-hidden flex flex-col gap-2 md:gap-4 p-2 md:p-4">
        ...
        {page === 'calendar' && specialists.length > 1 && (
          <div className="md:hidden flex gap-2 overflow-x-auto pb-1 flex-shrink-0 scrollbar-none">
            <button
              onClick={() => setSelectedSpecialistId(null)}
              className={clsx('px-3 py-1.5 rounded-full text-xs whitespace-nowrap border font-medium transition-colors flex-shrink-0',
                !selectedSpecialistId ? 'bg-brand-500 text-white border-brand-500' : 'bg-white text-slate-600 border-slate-200')}>
              All
            </button>
            {specialists.map(sp => (
              <button key={sp.id}
                onClick={() => setSelectedSpecialistId(sp.id === selectedSpecialistId ? null : sp.id)}
                className={clsx('px-3 py-1.5 rounded-full text-xs whitespace-nowrap border flex items-center gap-1.5 font-medium transition-colors flex-shrink-0',
                  selectedSpecialistId === sp.id ? 'bg-brand-500 text-white border-brand-500' : 'bg-white text-slate-600 border-slate-200')}>
                <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: sp.color }} />
                {sp.name.split(' ')[0]}
              </button>
            ))}
