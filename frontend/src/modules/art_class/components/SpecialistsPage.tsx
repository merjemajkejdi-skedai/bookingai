...
interface Props { specialists: Specialist[]; loading: boolean; onRefresh: () => void; }
    ...
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-0.5">{specialists.length} teachers</p>
        </div>
        <Button onClick={() => setShowAdd(true)}><Plus size={14} /> Add teacher</Button>
      </div>
      ...
      <div className="grid gap-3">
          ...
          <div key={sp.id} className="bg-white rounded-xl border border-slate-200 px-5 py-4 flex items-center gap-4">
            ...
            <div className="hidden sm:flex gap-1">
              {DAYS.map((d, i) => {
                ...
                return (
                  <span key={d} className={clsx('w-7 h-7 rounded-md text-xs flex items-center justify-center font-medium',
            ...
            <div className="flex gap-2">
              <Button variant="ghost" size="sm" onClick={() => setEditingHours(sp)}><Clock size={13} /> Hours</Button>
              <Button variant="ghost" size="sm" onClick={() => setEditing(sp)}><Pencil size={13} /> Edit</Button>
            </div>
          </div>
        ))}
        {specialists.length === 0 && (
          <div className="text-center py-12 text-slate-400">
            ...
            <Button variant="outline" size="sm" className="mt-3" onClick={() => setShowAdd(true)}>Add your first teacher</Button>
          </div>
        )}
      </div>

      {(showAdd || editing) && (
        <TeacherForm specialist={editing}
          onClose={() => { setShowAdd(false); setEditing(null); }}
          onSaved={() => { setShowAdd(false); setEditing(null); onRefresh(); }} />
      )}
      {editingHours && (
        <WorkingHoursModal specialist={editingHours}
...
function TeacherForm({ specialist, onClose, onSaved }: {
  ...
  async function save() {
    ...
    setSaving(true);
    try {
      if (specialist) { await api.updateSpecialist(specialist.id, { name, role, color }); }
      else { await api.createSpecialist({ name, role, color, tenantId: '', isActive: true }); }
      onSaved();
  ...
  return (
    <Modal title={specialist ? 'Edit teacher' : 'Add teacher'} onClose={onClose}>
      <div className="space-y-4">
        <Input label="Full name" value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Maria Kola" />
        <Input label="Specialty" value={role} onChange={e => setRole(e.target.value)} placeholder="e.g. Watercolour Instructor" />
...
function WorkingHoursModal({ specialist, onClose, onSaved }: {
  ...
  const [hours, setHours] = useState(
    DAYS.map((_, i) => {
      ...
      return { dayOfWeek: i, startTime: wh?.startTime ?? '09:00', endTime: wh?.endTime ?? '18:00', isWorking: wh?.isWorking ?? (i > 0 && i < 6) };
    })
  );
  const [saving, setSaving] = useState(false);

  function toggle(i: number) { setHours(h => h.map((d, idx) => idx === i ? { ...d, isWorking: !d.isWorking } : d)); }
  ...
  return (
    <Modal title={`${specialist.name} — Schedule`} onClose={onClose} wide>
      <div className="space-y-2">
        {hours.map((h, i) => (
          <div key={i} className={clsx('flex items-center gap-3 px-3 py-2.5 rounded-xl transition-colors',
            ...
            <span className={clsx('w-10 text-sm font-medium', h.isWorking ? 'text-slate-700' : 'text-slate-400')}>{DAYS[i]}</span>
            {h.isWorking ? (
              ...
              <span className="text-sm text-slate-400 flex-1">Not available</span>
            )}
          </div>
        ))}
      </div>
