...
// ── SPECIALISTS ──────────────────────────────────────────────────────────────
bookingRouter.get('/specialists', async (req: Request, res: Response) => {
...
bookingRouter.post('/specialists', async (req: Request, res: Response) => {
...
bookingRouter.put('/specialists/:id', async (req: Request, res: Response) => {
  const parsed = z.object({
...
bookingRouter.put('/specialists/:id/working-hours', async (req: Request, res: Response) => {
...
// ── SERVICE GROUPS ────────────────────────────────────────────────────────────
bookingRouter.get('/service-groups', async (req: Request, res: Response) => {
  const tenantId = req.query.tenantId as string || 'tenant-demo-001';
...
bookingRouter.post('/service-groups', async (req: Request, res: Response) => {
  const parsed = z.object({
    tenantId: z.string().default('tenant-demo-001'),
...
bookingRouter.put('/service-groups/:id', async (req: Request, res: Response) => {
...
bookingRouter.delete('/service-groups/:id', async (req: Request, res: Response) => {
...
bookingRouter.put('/bookings/:id', async (req: Request, res: Response) => {
  const parsed = z.object({
  ...
  if (status === 'cancelled' && updated?.customer_phone) {
    ...
    const msg = `Hi ${updated.customer_name}, your ${updated.service_name} appointment with ${updated.specialist_name} on ${dateStr} at ${timeStr} has been cancelled.${reasonPart} We apologise for any inconvenience.`;
  ...
  if (startsAt && status !== 'cancelled' && updated?.customer_phone) {
    ...
    const msg = `Hi ${updated.customer_name}, your ${updated.service_name} appointment with ${updated.specialist_name} has been rescheduled to ${newDateStr} at ${newTimeStr}.`;
    notifyCustomer(updated.customer_phone, msg);
  }
  ok(res, updated);
});

bookingRouter.delete('/bookings/:id', async (req: Request, res: Response) => {
  ...
  if (booking.customer_phone) {
    ...
    const reasonPart = reason ? `\nReason: ${reason}` : '';
    const msg = `Hi ${booking.customer_name}, your ${booking.service_name} appointment with ${booking.specialist_name} on ${dateStr} at ${timeStr} has been cancelled.${reasonPart} We apologise for any inconvenience.`;
    notifyCustomer(booking.customer_phone, msg);
  }
  ok(res, { id: req.params.id });
});

bookingRouter.delete('/bookings/:id/series', async (req: Request, res: Response) => {
  const { reason } = req.body as { reason?: string };
  const booking = await dbGet(`
    SELECT b.*, s.name AS specialist_name, sv.name AS service_name
    FROM bookings b JOIN specialists s ON s.id=b.specialist_id JOIN services sv ON sv.id=b.service_id
...
bookingRouter.get('/availability/suggest', async (req: Request, res: Response) => {
