...
// ── GET /events ───────────────────────────────────────────────────────────────
eventsRouter.get('/events', requireAuth, async (req: Request, res: Response) => {
...
// ── POST /events ──────────────────────────────────────────────────────────────
eventsRouter.post('/events', requireAuth, async (req: Request, res: Response) => {
  const tenantId = (req as any).user.tenantId;
...
// ── PUT /events/:id ───────────────────────────────────────────────────────────
eventsRouter.put('/events/:id', requireAuth, async (req: Request, res: Response) => {
  const tenantId = (req as any).user.tenantId;
  const { id } = req.params;
  const { title, description, date, startTime, endTime, teacherId, ageMin, ageMax, maxCapacity } = req.body;
  ...
  try {
    await dbRun(
      ...
      teacherId ?? null, ageMin ?? null, ageMax ?? null, maxCapacity ?? null,
      id
    );
    const row = await dbGet('SELECT * FROM art_events WHERE id=?', id) as any;
...
// ── DELETE /events/:id ────────────────────────────────────────────────────────
eventsRouter.delete('/events/:id', requireAuth, async (req: Request, res: Response) => {
  const tenantId = (req as any).user.tenantId;
  const { id } = req.params;
...
eventsRouter.get('/events/:id/registrations', requireAuth, async (req: Request, res: Response) => {
  const tenantId = (req as any).user.tenantId;
  const { id } = req.params;
...
eventsRouter.post('/events/:id/registrations', requireAuth, async (req: Request, res: Response) => {
  const tenantId = (req as any).user.tenantId;
  const { id } = req.params;
  ...
  if (event.max_capacity) {
    ...
    if (Number(cnt?.cnt || 0) >= event.max_capacity) {
      return err(res, 'This event is at full capacity');
    }
  }
...
eventsRouter.delete('/events/:id/registrations/:regId', requireAuth, async (req: Request, res: Response) => {
  const tenantId = (req as any).user.tenantId;
